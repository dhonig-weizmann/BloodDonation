function [breaths, dataTwoChannelsCleared] = TraceToBreaths_mustache(dataTwoChannels, Fs, varargin)
...
%
%   2023/08/01 - Lior .G.
%   2025/07/10- Timna (replaced left and right according to mustache data and removed the correct zeroing)dataTwoChannelsCleared = dataTwoChannels; %- possible_zero_offset_by_channel;
...
peakLeftInEachBlock = num2cell(arrayfun(@(st) max(dataTwoChannelsCleared(st.PixelIdxList, 2)), breaths));
volumeLeftInEachBlock = num2cell(arrayfun(@(st) sum(dataTwoChannelsCleared(st.PixelIdxList, 2)), breaths) / Fs);
peakRightInEachBlock = num2cell(arrayfun(@(st) max(dataTwoChannelsCleared(st.PixelIdxList, 1)), breaths));
volumeRightInEachBlock = num2cell(arrayfun(@(st) sum(dataTwoChannelsCleared(st.PixelIdxList, 1)), breaths) / Fs);
